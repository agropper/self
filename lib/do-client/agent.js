...
export class AgentClient {
  ...
  async create(options) {
    ...
    // Explicitly set temperature even if 0 (some APIs ignore 0, so we must be explicit)
    if (temperature !== undefined) {
      agentData.temperature = temperature;
      // Log to verify 0 is being sent (not omitted)
      if (temperature === 0) {
        console.log(`[AGENT CREATE] Explicitly setting temperature: 0`);
      }
    }
    if (k !== undefined) {
      agentData.k = k;
